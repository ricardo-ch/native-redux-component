...
module.exports = function () {
  function ReduxComponent(store) {
    var _this = this;

    var renderStateNames = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
    ...
    store.subscribe(function () {
      var newState = _.cloneDeep(store.getState());
      if (_.isEmpty(stateNames) && _.isEmpty(renderStateNames)) {
        _this.preRender(newState);
        return;
      }
      _.forEach(stateNames, function (name) {
        return _this.preSetState(newState, name);
      });
      if (!_.isEmpty(renderStateNames.filter(function (name) {
        return _this.preRender(newState, name);
      }))) _this.render();
    });
  }
  ...
  _createClass(ReduxComponent, [{
    key: 'preRender',
    value: function preRender(newState) {
      ...
      var property = _.head(_.at(this.state, name));
      var newProperty = _.head(_.at(newState, name));
    ...
    key: 'preSetState',
    value: function preSetState(newState, name) {
      var property = _.head(_.at(this.state, name));
      var newProperty = _.head(_.at(newState, name));
    ...
    */
    ...
    key: 'initOnChangeEvent',
    value: function initOnChangeEvent(selector, actionCreator) {
      var _this2 = this;

      var validate = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : function (value) {
        return value;
      };
      ...
      for (var i = 0; i < items.length; i += 1) {
        items[i].addEventListener('change', function (event) {
          ...
          if (value) {
            _this2.dispatch(actionCreator(value));
          }
        });
      }
    }
